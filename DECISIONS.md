...
**Amendment 2 (same-day, 2026-04-14, pre-task-#3-compute): Scenario revision for option (c) rendering choice.**

Task #3's design work (preflight `preflight/2026-04-14_day12_task3_preflight.txt` Section 3) reaches option (c) — COLMAP MVS reported on geometric metrics (chamfer / accuracy / completeness / F@k), 3DGS reported on novel-view synthesis metrics (PSNR / SSIM / LPIPS), dashes in non-applicable cells. Reasoning sequence: (a) dedicated renderer doubles code surface and introduces a renderer-choice confound; (b) `train_gsplat(n_iterations=0)` evaluates a 3DGS-zero-iter model, not COLMAP MVS — pushed back in the original DECISIONS 29 design-review discussion; (c) uses V1's existing evaluation pipeline unmodified (`experiments/run_baseline.py` + `src/simtorecon/evaluation/metrics.py`) and is the "don't force apples-to-apples where none exists" framing from the post-Day-9 plan Section 2 Day 12 task 5.

**Scenario-coverage implication of option (c).** DECISIONS 29's committed S_X1 through S_X6 were written assuming the MVS cell would have PSNR/SSIM/LPIPS values comparable to the 3DGS rows. Under option (c), that assumption is false for 4 of the 6 scenarios (S_X1, S_X2, S_X3, S_X5 — all assume a cross-method PSNR ordering that won't exist because MVS has no PSNR under option (c)).

**Revised scenario set (supersedes S_X1–S_X6 as committed).**

- **S_X1 (expected three-regime outcome).** Each method's native-metric result lands in its expected range. COLMAP MVS chamfer/F-score are benchmark-loose but V1-internally-consistent for dense MVS on scan9 (chamfer ~10–25 mm anchored to V1's 18.89 mm; F@5mm ~0.15–0.40 anchored to V1's 0.286). Frozen 3DGS matches Day 10's 3-seed median (21.32 ± 1.53 dB PSNR, SSIM 0.813, LPIPS 0.194). Over-dens 3DGS matches Day 10's 3-seed median (18.50 ± 3.20 dB PSNR, SSIM 0.884, LPIPS 0.087). Writeup = "three methods, three distinct failure modes (missing geometry, under-fit, over-fit pixel noise), each characterized at its native metric; the cross-method framing is at the mechanism level, not at a forced-overlap single-metric level."

- **S_X2 (MVS chamfer meaningfully worse than V1's seed=42 baseline).** Day 11's fused PLY at seed=123 SfM + seed=42 dense rerun produces notably higher chamfer than V1's 18.89 mm anchor. Caveat: V1's anchor is from scan9_v49_s42_a602e1 (different SfM seed), so a chamfer delta is confounded with the SfM seed difference, not a pure view-count effect. Writeup = "Day 11's MVS cell chamfer is X mm vs V1's 18.89 mm baseline, with the SfM seed confound noted; the delta is not a clean view-count-sensitivity measurement."

- **S_X3 (MVS chamfer meaningfully better than V1's baseline).** Day 11's chamfer comes in below 18.89 mm. Same SfM-seed caveat as S_X2. Writeup = "Day 11's seed=123 SfM produces a tighter dense reconstruction than V1's seed=42 baseline at the same nominal n=49 subsample; consistent with SfM-seed variance as a reconstruction-quality source."

- **S_X4 (unchanged from original DECISIONS 29).** Within-3DGS: over-dens > frozen on PSNR. Contradicts Day 9 + Day 10 + Day 11 chain. Low prior. Stop and investigate root cause — do NOT write up until identified.

- **S_X5 (MVS chamfer implausibly low or high — pipeline bug).** Chamfer < 5 mm or > 50 mm, OR F@5mm outside [0.10, 0.50], OR ICP fitness < 0.30. Hard stop, not a finding until root cause is identified. Candidate causes: ICP misalignment, wrong GT PLY downloaded, corrupt fused PLY, unit mismatch.

- **Pre-report diagnostic gate (added during task #3 design).** Separate from the S_X5 pipeline-bug hard stop. If chamfer lands in [5, 10] mm or [30, 50] mm — inside S_X5's "not a pipeline bug" range but outside the 2× envelope around V1's 18.89 mm anchor — halt and diagnose BEFORE reporting the number. The concern is that a chamfer in e.g. [5, 10] or [30, 50] could be either (a) a genuine method-comparison signal worth writing up or (b) pipeline drift (ICP alignment imperfection, coordinate-scale edge case, fused PLY corruption below the S_X5 threshold, GT mesh mismatch below the ICP fitness threshold). Cheap insurance: investigation is $0 local compute; silently shipping a pipeline-drift-contaminated number as a method-comparison signal is methodologically expensive. Diagnostic steps: (1) inspect ICP fitness and translation magnitudes, (2) verify PLY point count matches expected ~257k, (3) verify GT PLY is stl009_total.ply and contains expected scan9 scene, (4) sanity-check chamfer by visualizing the aligned PLYs side-by-side in Open3D. Report only after diagnosis concludes the number is legitimate, with the investigation notes appended to the Day 12 writeup. If in [10, 30] mm: proceed to S_X1 / S_X2 / S_X3 as normal.

- **S_X6 (unchanged from original DECISIONS 29).** Within-3DGS: perceptual-metric ordering flips (e.g., frozen beats over-dens on SSIM/LPIPS). Would contradict Day 9's PSNR-LPIPS-SSIM asymmetry finding. Plausible if fresh re-run diverges from Day 10; reconcile before writeup.

- **S_X7 (new).** Cross-method narrative holds at mechanism level regardless of specific numbers. All three methods' metrics land "numerically fine" (within native expected ranges) and the cross-method story is the three-regime framing at the mechanism level, not at any numeric ordering. The narrative carries because each failure mode is traceable to a pre-identified mechanism (V1 stress regime for MVS, gsplat under-init for frozen, gsplat over-densification for over-dens). This is the framing that makes option (c) coherent: the cross-method comparison is the mechanism taxonomy, not a single-metric horse race.

**Scenarios NOT applicable under option (c) (originally S_X1, S_X2, S_X3, S_X5 of the committed DECISIONS 29).** These assumed a cross-method PSNR ordering. Under option (c), MVS has no PSNR column; no cross-method PSNR ordering exists to compare. The scenarios are superseded by the revised set above.

**What this amendment preserves.**

- The three-regime frame (COLMAP MVS at n=37, frozen 3DGS at 9k init, over-dens 3DGS at 9k init) is unchanged.
- The Day 11 provenance sentence (dense-init bounding enriched the frame from {classical, neural} to {classical, frozen, over-dens}) is unchanged.
- The "what this pre-commit is NOT" section is unchanged.
- The S_X3 "orthogonal to headline" pre-commit from the original DECISIONS 29 (that the V1.5 headline's scope precludes method comparison from falsifying it) is unchanged — it applied to a hypothetical cross-method PSNR ordering that no longer exists, but the underlying contract (headline is within-3DGS densification, not cross-method) is preserved and carries forward into S_X7's narrative framing.
- Hard stops are preserved as a *metric-type-aware* set: the geometric-metric hard stops apply to the MVS row (S_X5 above), the novel-view synthesis hard stops apply to the 3DGS rows (Day 10 anchor deviation > 0.5 dB).

**Reporting posture (pre-committed).**

- **3DGS rows report 3-seed median ± range.** V1.5's default multi-seed discipline applies (median and range across seeds {42, 123, 7}, same as Day 10's sweep). "Range" here is `max − min across the 3 seeds`, not standard deviation — Day 10's convention per `results/gsplat/multiseed/` and DECISIONS 25's reporting. On a 3-seed sample, range and standard deviation differ substantially; explicit convention beats reader inference.

- **Matched-seed-42 values are NOT the table cells.** Day 11 used matched-seed-42 (frozen 22.56 dB, over-dens 18.90 dB) to isolate the dense-init regime effect from seed variance. Day 12's cross-method table isn't isolating a regime effect; it's reporting method-level comparisons. The matched-seed-42 values live in a short footnote on the table — "3DGS cells report 3-seed median ± range per V1.5 multi-seed discipline. Day 11 regime-effect isolation used matched-seed-42 values (frozen 22.56, over-dens 18.90) for single-variable discipline; those values are not the appropriate cross-method comparison basis." — not in the cells themselves.

- **MVS row reports a single-run value with honest asymmetry.** Day 11 measured source-(d) CUDA-thread stochasticity at 0.10 dB on downstream 3DGS PSNR — that is a measurement of how PatchMatch variance propagates through 3DGS training, NOT a measurement of how PatchMatch variance propagates through chamfer or F-score directly. Those are different metric spaces. A 2-run MVS chamfer range would be a 2-point sample, which DECISIONS 28's single-seed generalization gap argues against reporting as a distribution. Day 12 ships the single-run chamfer with a caveat: "3DGS multi-seeded per V1.5 discipline (3 seeds, range reported); MVS single-run per task #3 scope. Source-(d) variance on downstream 3DGS PSNR was measured at Day 11 at 0.10 dB, but geometric-metric-level variance for the MVS cell is uncharacterized and a 2-run sample would overclaim precision relative to the 3DGS row's 3-seed distribution." This is an honest asymmetry — the alternative (2-run MVS for symmetry) is a fake distribution, which is worse than an acknowledged single-run.

**What this amendment does NOT preserve.**
